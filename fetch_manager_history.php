...
<div class="modal-body p-4 bg-light " >
        ...
        <div class="card-body py-3 px-4">
            <div class="row align-items-center">
                <div class="col-md-4">
                    ...
                    <span class="fw-bold text-dark"><?= $citizen['national_id']; ?></span>
                </div>
                <div class="col-md-4">
                    <small class="text-muted d-block">رقم الهاتف:</small>
                    <span class="fw-bold text-success"><?= $citizen['phone']; ?></span>
                </div>
                <div class="col-md-4 text-start">
                    <span class="badge bg-danger rounded-pill px-3"><?= count($appointments); ?> طلبات سابقة</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <h6 class="fw-bold text-dark mb-3">
        <i class="fas fa-list-ul text-success me-2"></i> سجل الطلبات والوثائق:
    </h6>
    
    <!-- Accordion سجل الطلبات -->
                ...
                <div class="accordion-item border-0 border-bottom">
                        ...
                        <button class="accordion-button collapsed py-3" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_<?= $index; ?>">
                            <div class="d-flex justify-content-between w-100 align-items-center pe-3">
                                <span class="fw-bold small text-dark">
                                    <i class="far fa-calendar-alt me-2 text-success"></i> <?= date('Y-m-d', strtotime($app['created_at'])); ?>
                                </span>
                                ...
                                ?>
                                <span class="badge bg-<?= $status['class']; ?> rounded-pill px-3 shadow-sm" style="font-size: 11px;"><?= $status['label']; ?></span>
                            </div>
                        </button>
                    </h2>
                    <div id="collapse_<?= $index; ?>" class="accordion-collapse collapse" data-bs-parent="#historyAccordion">
                        <div class="accordion-body bg-white p-4">
                            <div class="row g-4">
                                <!-- سبب الطلب ورد المدير -->
                                <div class="col-md-5 border-start">
                                    <label class="fw-bold text-success small d-block mb-2">سبب الطلب:</label>
                                    <div class="p-3 bg-light rounded-3 mb-3 small"><?= nl2br(htmlspecialchars($app['message'])); ?></div>
                                    
                                    <label class="fw-bold text-primary small d-block mb-2">رد المدير:</label>
                                    <div class="p-3 bg-white border rounded-3 small shadow-sm">
                                ...
                                <div class="col-md-7 text-center border-end">
                                    <label class="fw-bold text-secondary small d-block mb-2 ">المرفقات:</label>
                                        ...
                                        <div class="rounded border bg-light overflow-hidden shadow-sm" style="height: 350px;">
                                            ...
                                            <?php else: ?>
                                                <div class="d-flex flex-column align-items-center justify-content-center h-100 p-4 text-muted">
                                                    <i class="fas fa-file-download fa-3x mb-2 text-success"></i>
                                                    <p>ملف <?= strtoupper($ext); ?> لا يدعم المعاينة</p>
                                                    <a href="<?= $app['files_path']; ?>" class="btn btn-success btn-sm rounded-pill px-4" download>تحميل المستند</a>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    <?php else: ?>
                                        <div class="alert alert-light border-dashed py-5 text-muted small text-center">
                                            <i class="fas fa-file-alt d-block fa-3x mb-2 opacity-25"></i>
                                            لا توجد مرفقات.
                                        </div>
                                    <?php endif; ?>
                                </div>
...
<div class="modal-footer border-0 bg-white shadow-sm">
    <button type="button" class="btn btn-danger px-4 py-2 rounded-3 fw-bold shadow-sm" data-bs-dismiss="modal">إغلاق الأرشيف</button>
</div>